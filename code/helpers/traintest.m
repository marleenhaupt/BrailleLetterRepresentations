...
for iTime = 1:length(timewindow) % size of data
    ...
    model        = svmtrain(labels_train, train_data,'-s 0 -t 0 -q'); %was libsvm
    
    % test
    test_data = [squeeze(testA(end,:,iTime)) ; squeeze(testB(end,:,iTime)) ];
    [predicted_label, l_accuracy, decision_values] = svmpredict(labels_test, test_data, model,'-q'); %waslibsvm
