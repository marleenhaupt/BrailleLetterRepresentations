function bootstrapping_onsets_timedec(within,across,diff)
...
figure
within = plot(mean(data_W_base),'Color',c3, 'LineWidth', 2)
hold on
...
hold on
plot([CI_95_W(1)+10;CI_95_W(2)+10],[y_CI_W;y_CI_W],'Color',c3, 'LineWidth', 2) 
hold on
plot(nanmean(onset_latency_samples_W)+10,y_CI_W,'.','MarkerSize',10,'Color',c3)
hold on
upper = mean(data_W_base) + SEM_W;
lower = mean(data_W_base) - SEM_W;
inBetween = [upper, fliplr(lower)];
fill(x2, inBetween, c3, 'FaceAlpha', 0.155, 'LineStyle', 'none');
hold on;
...
ylim([-5,25])
set(gca,'box','off')
legend([within diff across],'within','within-across', 'across')
legend('boxoff')
